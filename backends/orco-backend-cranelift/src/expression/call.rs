    ...
    pub fn build_call_expression(
        ...
        match call.expression.as_ref() {
            orco::ir::Expression::Symbol(symbol) => {
                let function = if symbol.as_any_function().is_some() {
                    ...
                    panic!("Can only call functions, not '{}'. Operator overloads should've been replaced by now, did you run type checking/inference?", call.expression)
                };
                let args = call
                    .args
                    .iter()
